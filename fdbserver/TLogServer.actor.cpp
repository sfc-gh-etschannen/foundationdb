...
ACTOR Future<Void> updateLogSystem(TLogData* self, Reference<LogData> logData, LogSystemConfig recoverFrom, Reference<AsyncVar<Reference<ILogSystem>>> logSystem) {
	loop {
		bool found = self->dbInfo->get().client.logSystemConfig.recruitmentID == logData->recruitmentID;
		if (found) {
		...
		for (const auto& it : self->dbInfo->get().client.logSystemConfig.oldTLogs) {
